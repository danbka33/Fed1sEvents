...
function util.transfer_inventory_filters_direct(inv_a, inv_b)
    if inv_a and inv_b and inv_a.supports_filters() and inv_b.supports_filters() then
        ...
        if inv_a.supports_bar() and inv_b.supports_bar() then
            ...
            if bar <= #inv_a then
                -- Bar equals size+1 when it is unset
...
function util.swap_fluids (entity_a, entity_b)
    if not (entity_a.fluidbox and entity_b.fluidbox) then
        return
    end
...
function util.position_equal(position1, position2, epsilon)
    ...
    if math.abs(position1.x - position2.x) > epsilon then
        return false
    end
    if math.abs(position1.y - position2.y) > epsilon then
        return false
    end
    return true
end
...
function util.intersects_ray_aabb(ray_origin, orientation, aabb)
    ...
    if (ray_vector.x ~= 0) then
        ...
        -- ray is going exactly vertically
    ...
    if (ray_vector.y ~= 0) then
        ...
        -- ray is going exactly horizontally
...
function util.nvectors_delta(a, b)
    -- from a to b
    local delta = {}
...
function util.signal_from_wires(red, green, signal)
    local value = 0
    if red then
        value = value + (red.get_signal(signal) or 0)
    end
    if green then
        value = value + (green.get_signal(signal) or 0)
    end
    return value
end
...
function util.conditional_mark_for_deconstruction(entities, surface, position)
    ...
    local networks = surface.find_logistic_networks_by_construction_area(position, playerForce)
    for _, network in pairs(networks) do
        if network.robot_limit > 1000000 then
            for _, entity in pairs(entities) do
                entity.order_deconstruction(playerForce)
            end
            break
        end
...
function util.settings_pasted(event, names, serialize_fn, deserialize_fn, cb)
    ...
    if player_index and game.get_player(player_index) and game.get_player(player_index).connected
        ...
        if not util.table_contains(names, event.source.name) then
            return
        end
        if not util.table_contains(names, event.destination.name) then
            return
        end
        ...
        if tags then
            ...
            if cb then
                cb(event.destination, player_index)
            end
        end
    end
end
...
function util.delete_tags(element, tags_to_delete)
    local tags = element.tags
    for _, tag in pairs(tags_to_delete) do
        tags[tag] = nil
    end
    element.tags = tags
end
