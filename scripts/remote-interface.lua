...
remote.add_interface(
        "fed1s-event",
        {
            --/c remote.call("fed1s-event", "begin_meteor_shower", {target_entity = game.player, meteors = 10})
            --/c remote.call("fed1s-event", "begin_meteor_shower", {target_entity = game.player.selected or game.player})
            ...
            begin_meteor_shower = function(data)
                ...
                if entity then
                    ...
                    if surface then
                        Meteor.begin_meteor_shower(surface.index, entity.position, data.range, data.meteors)
                    end
                elseif data.surface_index then
                    local surface = game.surfaces[data.surface_index]
                    if surface then
                        local position = data.position or {x = 0, y = 0}
                        Meteor.begin_meteor_shower(surface.index, position, data.range, data.meteors)
                    end
                end
            end,
